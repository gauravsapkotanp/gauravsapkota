...
const CustomDataTable = ({ data, columns, customRenderFunction, actions = [] }) => {
    ...
    const renderField = (column, item) => {
        if (column.key === 'photopath' && item[column.key]) {
            if (customRenderFunction) {
                return customRenderFunction(item);
            } else {
                return <img className="w-36 mx-auto rounded-xl" src={item[column.key]} alt="" />;
            }
        } else {
    ...
    useEffect(() => {
        ...
        const endEntry = Math.min((currentPage + 1) * itemsPerPage, filteredData.length);
            ...
            <div className="overflow-auto border-2 rounded-xl border-gray-300">
                <div className="flex justify-between items-center p-4">
                    <div className="flex gap-2">
                        ...
                        <button onClick={exportToPDF} className="m-2 p-2 bg-white text-red-700 border-2 border-red-700 hover:border-red-500 hover:text-red-500 rounded-lg"><FaFilePdf className='text-xl' /></button>
                    </div>
                    <div>
                ...
                <table className="w-full text-center whitespace-nowrap">
                    <thead>
                        <tr>
                            <th className="font-bold py-2 border-r border-b border-t border-gray-300 px-2 text-black">S.N</th>
                            {columns.map((column) => (
                                <th key={column.key} className="font-bold py-2 border-r border-b border-t border-gray-300 px-2 text-black">{column.label}</th>
                            ))}
                            <th className="font-extrabold py-2 border-b border-t border-gray-300 px-2 text-black">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        {displayedData.map((item, index) => (
                            <tr key={item.id} className="rounded-bl-xl">
                                <td className="py-2 border-r border-t border-b border-gray-300 px-2 text-black">{currentPage * itemsPerPage + index + 1}</td>
                                {columns.map((column) => (
                                    <td key={column.key} className="py-2 border-r border-t border-b border-gray-300 px-2 text-black">
