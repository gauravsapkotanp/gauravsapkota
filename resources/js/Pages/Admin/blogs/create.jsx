...
const Create = () => {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    router.post('/blogs/store', values);
  }
      ...
      <AdminLayout>
        ...
        <div className="p-4 mx-3 lg:mx-5 overflow-x-auto overflow-y-hidden bg-white rounded-xl shadow-md -mt-28">
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              <label htmlFor="photopath" className="  text-white hover:bg-transparent border-2 bg-orange-500 hover:text-red-500 hover:border-primary duration-1000 hover:text-primary py-2 px-4  rounded-lg cursor-pointer">Choose Image</label>
              <input  hidden  type="file"   id="photopath" accept="image/*" onChange={handleImageChange} required  />
            </div>
            {selectedImage && (
              <img src={URL.createObjectURL(selectedImage)} alt="Selected" className="mb-4 rounded-xl   h-full" style={{ maxWidth: "400px" }} />
            )}
            <button type="submit" className="bg-blue-500 hover:bg-blue-700 mt-10 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Submit</button>
          </form>
        </div>
      </AdminLayout>
    </>
